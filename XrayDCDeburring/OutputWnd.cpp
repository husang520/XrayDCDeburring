...
{
	m_pFireOutputDlg = NULL;
	this->SetMinSize(CSize(100, 100));
}

COutputWnd::~COutputWnd()
{
	if (m_pFireOutputDlg != NULL)
	{
		if (m_pFireOutputDlg->GetSafeHwnd() != NULL)
		{
			m_pFireOutputDlg->DestroyWindow();
		}
		delete m_pFireOutputDlg;
		m_pFireOutputDlg = NULL;
	}
}
...
int COutputWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_pFireOutputDlg = new CFireOutputDlg();
	if (!m_pFireOutputDlg->Create(IDD_DIALOG_FIRE_OUTPUT, this))
	{
		TRACE0("未能创建输出窗口\n");
		return -1;      // 未能创建
	}
	m_pFireOutputDlg->ShowWindow(SW_SHOW);
...
void COutputWnd::OnSize(UINT nType, int cx, int cy)
{
	...
	if (pMainframe != NULL && pMainframe->GetSafeHwnd() != NULL)
	{
		CRect outWindowRect;
		this->GetWindowRect(outWindowRect);
		if (outWindowRect.Height() < 400)
		{
			outWindowRect.top = outWindowRect.bottom - 400;
			this->MoveWindow(outWindowRect);
		}
		
	
	}
	AdjustLayout();
	if (m_pFireOutputDlg != NULL && m_pFireOutputDlg->GetSafeHwnd() != NULL)
	{
		CRect rClientRect;
		GetClientRect(rClientRect);
		m_pFireOutputDlg->MoveWindow(rClientRect);
		//m_pFireOutputDlg->ShowWindow(SW_SHOW);
	}
...
CFireOutputDlg* COutputWnd::GetFireOutPutWnd()
{
	return m_pFireOutputDlg;
}
