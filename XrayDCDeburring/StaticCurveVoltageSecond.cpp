// StaticCurveVoltageSecond.cpp : 实现文件
//
...
IMPLEMENT_DYNAMIC(CStaticCurveVoltageSecond, CStatic)

CStaticCurveVoltageSecond::CStaticCurveVoltageSecond()
{
	m_fPixelValueUp = 1.0;
	m_fPixelValueDn = 1.0;
...
BEGIN_MESSAGE_MAP(CStaticCurveVoltageSecond, CStatic)
	ON_WM_MOUSEMOVE()
	ON_WM_PAINT()
	ON_WM_LBUTTONUP()
	ON_WM_LBUTTONDOWN()
...
// CStaticCurveVoltageSecond 消息处理程序




void CStaticCurveVoltageSecond::OnMouseMove(UINT nFlags, CPoint point)
{
...
void CStaticCurveVoltageSecond::OnPaint()
{
...
void CStaticCurveVoltageSecond::DrawSelectBox(CDC* pDC, CRect cRect)
{
	//画选择框
	if (m_bSelectAreaMode)
	{
...
void CStaticCurveVoltageSecond::DrawGraph(CDC * pDC, CRect cRect)
{
	...
	//}
	int nSumSeconds = 0;
	for (int i = 0; i < m_struct_Program.VectProgItem.size(); i++)
	{
		if (m_struct_Program.VectProgItem.at(i).nHightVotage > m_nMaxVoltage)
		{
			m_nMaxVoltage = m_struct_Program.VectProgItem.at(i).nHightVotage;
		}
		nSumSeconds += m_struct_Program.VectProgItem.at(i).nKeepTime;
	}
	maxValue = ceil(m_nMaxVoltage /10.0)*10;
	minValue = 0;
	double nSpanValue = maxValue - minValue;
	...
	for (int i = 0; i < 6; i++)
	{
		...
		CString strText = L"";
		strText.Format(L"%.0f", minValue + maxValue /5*i);
	...
	int nValueSize = m_struct_Program.VectProgItem.size();

	int nMaxValueSize = ceil(nSumSeconds / 10.0) * 10;
	if (nValueSize == 0)
	{
		nMaxValueSize = 10;
	}
	...
	fOnePixcelValue = (nWidth) / (nSumSeconds * 1.0);
	...
	CPoint PointTemp;
	int nSumoffset = 0;
	CBrush brushAngle(BS_SOLID,RGB(0,0,255));
	CBrush *oldBrush = pDC->SelectObject(&brushAngle);

	for (int i = 0; i < m_struct_Program.VectProgItem.size(); i++)
	{
		int nseconds = m_struct_Program.VectProgItem.at(i).nKeepTime;
		int nVoltage = m_struct_Program.VectProgItem.at(i).nHightVotage;
		PointTemp.y = nBottom- nVoltage/ fonePixelValueY;
		PointTemp.x = 40+ nSumoffset * fOnePixcelValue;
		vect_pts.push_back(PointTemp);
		PointTemp.x = 40 + (nSumoffset + nseconds)* fOnePixcelValue;
		vect_pts.push_back(PointTemp);
		nSumoffset += nseconds;
		if (m_struct_Program.VectProgItem.at(i).nStep == m_nStep)
		{
			POINT triangle[3];
			
			triangle[0].x = (vect_pts.at(vect_pts.size()-1).x + vect_pts.at(vect_pts.size() - 2).x)/2; triangle[0].y = PointTemp.y-3; // 顶点1
			triangle[1].x = triangle[0].x-3; triangle[1].y = PointTemp.y+3; // 顶点2
			triangle[2].x = triangle[0].x + 3; triangle[2].y = PointTemp.y + 3; // 顶点3
...
void CStaticCurveVoltageSecond::DrawAllElement(CDC* pDC, CRect cRect)
{
	//画背景
	CBrush BkBrush;
	CPen   borderPen;
...
void CStaticCurveVoltageSecond::DrawGraphBackground(CDC * pDC, CRect cRect)
{
	TRIVERTEX vert[4];
...
void CStaticCurveVoltageSecond::OnLButtonUp(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
...
void CStaticCurveVoltageSecond::OnLButtonDown(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	if (m_bSelectAreaMode)
	{
...
void CStaticCurveVoltageSecond::OnMButtonDblClk(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
...
void CStaticCurveVoltageSecond::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	CRect clientRect;
	GetClientRect(clientRect);
