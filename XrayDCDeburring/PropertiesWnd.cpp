...
int CPropertiesWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_pDetectInfoInputDlg = new CDetectInfoInputDlg();
	if (!m_pDetectInfoInputDlg->Create(IDD_DIALOG_DETECTINFO_INPUT, this))
	{
		TRACE0("未能创建输出窗口\n");
		return -1;      // 未能创建
	}
	m_pDetectInfoInputDlg->ShowWindow(SW_SHOW);
	//CRect rectDummy;
	//rectDummy.SetRectEmpty();
...
void CPropertiesWnd::OnSize(UINT nType, int cx, int cy)
{
	...
	AdjustLayout();
	if (m_pDetectInfoInputDlg != NULL && m_pDetectInfoInputDlg->GetSafeHwnd() != NULL)
	{
		CRect rClientRect;
		GetClientRect(rClientRect);
		m_pDetectInfoInputDlg->MoveWindow(rClientRect);
	}
}
...
void CPropertiesWnd::UpdateProgramItem()
{
	m_pDetectInfoInputDlg->UpdateProgramItem();
}