...
bool CLog::InitLogFile()
{
	...
	wcscat_s(szPath, L"Log");
	SHCreateDirectory(NULL, szPath);
	SYSTEMTIME st;
	GetLocalTime(&st);
	swprintf_s(m_szLog, L"%s\\%d%02d%02d-%02d%02d%02d%03d.log", szPath, st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
	m_nWriteSize = 0;
	return true;
}
...
bool CLog::WriteLog(const char* lpLog)
{
	...
	::GetLocalTime(&st);
	char szTime[30] = { 0 };
	sprintf_s(szTime, "%d-%02d-%02d %02d:%02d:%02d.%03d\t", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
...
bool CLog::WriteLog(const char* lpFile, long lLine, const char* lpLog)
{
	...
	::GetLocalTime(&st);
	char szTime[30] = { 0 };
	sprintf_s(szTime, "%d-%02d-%02d %02d:%02d:%02d.%03d\n", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
	fwrite(szTime, 1, strlen(szTime), fp);
	if (NULL != lpFile)
